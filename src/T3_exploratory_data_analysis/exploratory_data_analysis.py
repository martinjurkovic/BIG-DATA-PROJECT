...
def main():
    ...
    df = read_files(
        base_path, file_format, usecols=["Registration State"], years=years
    ).compute()
    ...
    start_time = time.time()
    df = read_files(
        base_path, file_format, usecols=["Vehicle Body Type"], years=years
    ).compute()
    ...
    start_time = time.time()
    df = read_files(
        base_path, file_format, usecols=["Vehicle Year", "Feet From Curb"], years=years
    )
    df = df.dropna()
    ...
    start_time = time.time()
    df = read_files(
        base_path, file_format, usecols=["Violation Code"], years=years
    ).compute()
    df = df.reset_index(drop=True)
    ...
    start_time = time.time()
    df = read_files(
        base_path, file_format, usecols=["Violation Precinct"], years=years
    ).compute()
    df = df.reset_index(drop=True)
    ...
    start_time = time.time()
    df = read_files(
        base_path, file_format, usecols=["Vehicle Make"], years=years
    ).compute()
    df = df.reset_index(drop=True)
    ...
    start_time = time.time()
    df = read_files(
        base_path, file_format, usecols=["Issuing Agency"], years=years
    ).compute()
    df = df.reset_index(drop=True)
    ...
    start_time = time.time()
    df = read_files(
        base_path, file_format, usecols=["Violation Time"], years=years
    ).compute()
    df = df.reset_index(drop=True)
    plt.figure(figsize=(12, 6))
    ...
    df = read_files(
        base_path,
        file_format,
        usecols=["Violation Precinct", "Violation Location"],
        years=years,
    )
    violation_location_precinct = (
        df.groupby(["Violation Precinct", "Violation Location"])
        .size()
        .compute()
    ...
    start_time = time.time()
    df = read_files(
        base_path, file_format, usecols=["Feet From Curb"], years=years
    ).compute()
    plt.figure(figsize=(12, 6))
