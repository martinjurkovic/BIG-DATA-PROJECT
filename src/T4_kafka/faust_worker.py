...
highest_temperature_topic = app.topic(
    "highest_temperature", key_type=str, value_type=HighestTemperature
)

hourly_temperature_table = app.Table(
    "hourly_temperature", key_type=str, value_type=float, partitions=1, default=float
)
hourly_count_table = app.Table(
    "hourly_count", key_type=str, value_type=int, partitions=1, default=int
)
    ...
    async for key, record in data.items():
        ...
        if len(reference_values[key]) < N:
            ...
            if np.abs(air_temp) > MAX_TEMP:
                ...
                await outliers_topic.send(
                    key=key,
                    value={
                        "temp": np.round(air_temp, 2),
            ...
            if np.abs(z_score) > 3 and np.abs(z_score_proj) > 3:
                ...
                continue
            # update the momentum
            momentum = SMOOTHING_FACTOR * momentum + (1 - SMOOTHING_FACTOR) * (
                air_temp - reference_values[key][-1]
            )
...
async def compute_hourly_temperature():
    ...
    for station_name in ["Aleknagik_1_NNE", "Bethel_87_WNW", "Cordova_14_ESE"]:
        if (station_name, hour, date) in hourly_temperature_table:
            ...
            if temperature_count > 0:
                ...
                print(
                    f"Hourly mean temperature for station {station_name} @ {date}-{hour}: {hourly_mean_temperature}"
                )
                await hourly_data_topic.send(
                    key=station_name,
                    value={
                        "hour": hour,
                        "temperature": np.round(hourly_mean_temperature, 3),
                    },
                )
    ...
    if len(hourly_data):
        ...
        print(
            f"Hottest station at hour {CURRENT_HOUR} is {hottest_station} with temperature {hourly_data[hottest_station]}"
        )
        await highest_temperature_topic.send(
            key=hour,
            value={
                "station": hottest_station,
                "temperature": np.round(hourly_data[hottest_station], 4),
            },
        )
