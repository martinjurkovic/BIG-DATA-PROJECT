...
os.makedirs(exist_ok=True, name=os.path.join(DATA_DIR, "CSV", "augmented"))

# Load the school data
school_data = pd.read_csv(os.path.join(DATA_DIR, "CSV", "raw", "school_locations.csv"))
# Convert dates
...
for category in school_data["Location_Category_Description"].unique():
    school_data[school_data["Location_Category_Description"] == category].plot.scatter(
        x="LONGITUDE",
        y="LATITUDE",
        alpha=0.5,
        ax=ax,
...
for borough in school_data["Borough"].unique():
    ...
    hourly_df.to_csv(
        os.path.join(DATA_DIR, "CSV", "augmented", f"{borough}_school_openings.csv"),
        index=False,
    )
print()
...
def get_holiday_categories(date):
    ...
    ny_holidays = {
        ...
        "Mother's Day": pd.Timestamp(
            year=date.year,
            month=5,
            day=8 + (6 - pd.Timestamp(year=date.year, month=5, day=1).weekday()) % 7,
        ),
        "Father's Day": pd.Timestamp(
            year=date.year,
            month=6,
            day=15 + (6 - pd.Timestamp(year=date.year, month=6, day=1).weekday()) % 7,
        ),
    ...
    school_holidays = {
        ...
        "Labor Day": (
            pd.Timestamp(
                ...
                month=9,
                day=(
                    1 + (0 - pd.Timestamp(year=date.year, month=9, day=1).weekday()) % 7
                ),
            ),
            0,
        ),
        ...
        "Columbus Day": (
            pd.Timestamp(
                ...
                day=(
                    8
                    + (0 - pd.Timestamp(year=date.year, month=10, day=1).weekday()) % 7
                ),
            ),
            0,
        ),
        "Election Day": (
            pd.Timestamp(
                ...
                day=(
                    1
                    + (2 + pd.Timestamp(year=date.year, month=11, day=1).weekday()) % 7
                )
                + 1,
            ),
            0,
        ...
        "Thanksgiving Break": (
            pd.Timestamp(
                ...
                day=(
                    22
                    + (3 - pd.Timestamp(year=date.year, month=11, day=1).weekday()) % 7
                ),
            ),
            pd.Timestamp(
                year=date.year,
        ...
        "Dr. Martin Luther King Jr. Day": (
            pd.Timestamp(
                ...
                day=(
                    15
                    + (0 - pd.Timestamp(year=date.year, month=1, day=1).weekday()) % 7
                ),
            ),
            0,
        ),
        ...
        "Memorial Day": (
            pd.Timestamp(
                ...
                month=5,
                day=(
                    31 - (pd.Timestamp(year=date.year, month=5, day=31).weekday() - 0)
                ),
            ),
            0,
        ),
...
hourly_df.to_csv(
    os.path.join(DATA_DIR, "CSV", "augmented", "holidays.csv"), index=False
)
print()

# Business data
business_data = pd.read_csv(
    os.path.join(DATA_DIR, "CSV", "raw", "business_locations.csv"), low_memory=False
)

# Convert License Creation Date and License Expiration Date to datetime
business_data["License Creation Date"] = pd.to_datetime(
    business_data["License Creation Date"], errors="coerce"
)
business_data["License Expiration Date"] = pd.to_datetime(
...
business_data["LATITUDE"] = business_data["Location"].apply(
    lambda x: float(x.split(",")[0])
)
business_data["LONGITUDE"] = business_data["Location"].apply(
    lambda x: float(x.split(",")[1])
)
# business_data.drop(columns=['Location'], inplace=True)

business_data = business_data.loc[
    business_data["License Expiration Date"] > pd.Timestamp("2013-01-01")
]
...
def get_business_hourly_df(business_data, borough):
    ...
    for industry, hours in industry_hours.items():
        ...
        if not industry_weekend_service[industry]:
            industry_filter = industry_filter & hourly_df["datetime"].dt.weekday.isin(
                range(0, 5)
            )
...
for borough in business_data["Borough"].unique():
    ...
    hourly_df.to_csv(
        os.path.join(DATA_DIR, "CSV", "augmented", f"{borough}_business_openings.csv"),
        index=False,
    )
print()
...
event_data = pd.read_csv(os.path.join(DATA_DIR, "CSV", "raw", "events.csv"))

# Convert Start Date/Time and End Date/Time to datetime
event_data["Start Date/Time"] = pd.to_datetime(
    event_data["Start Date/Time"], errors="coerce"
)
event_data["End Date/Time"] = pd.to_datetime(
...
for borough in event_data["Event Borough"].unique():
    ...
    hourly_df.to_csv(
        os.path.join(DATA_DIR, "CSV", "augmented", f"{borough}_events.csv"), index=False
    )
print()
...
landmark_data = pd.read_csv(os.path.join(DATA_DIR, "CSV", "raw", "landmarks.csv"))
...
for borough in landmark_data["Borough"].unique():
    ...
    hourly_df.to_csv(
        os.path.join(DATA_DIR, "CSV", "augmented", f"{borough}_landmarks.csv"),
        index=False,
    )
